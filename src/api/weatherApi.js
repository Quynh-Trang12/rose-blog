...
// Base URLs for the two external APIs used by the weather widget.
 ...
 * Sends a GET request to the Open-Meteo forecast endpoint
...
export const fetchCurrentWeather = async (lat, lon) => {
  ...
  // Return only the current_weather payload, discarding metadata.
  return data.current_weather
}

/**
 * Reverse-geocodes a coordinate pair to a human-readable city display string.
 * Uses the BigDataCloud reverse geocoding API to convert
...
export const fetchCityName = async (lat, lon) => {
  ...
  // Check if the geocoding service responded successfully.
  if (!res.ok) throw new Error(`Geocoding API error: ${res.status}`)

  const data = await res.json()
  // Attempts to find the best locality name from the response.
