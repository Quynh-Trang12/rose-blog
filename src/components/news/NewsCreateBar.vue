...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      this.error = ''
      if (!this.title.trim()) { this.error = 'A title is required to publish.'; return }
      if (!this.content.trim() || this.content === '<p></p>') { this.error = 'Content cannot be empty.'; return }
...
<template>
  ...
  >
    ...
    >
      <img :src="safeAvatar" class="rounded-circle shadow-sm border border-white" width="45" height="45" alt="User avatar" />
      <div class="flex-grow-1">
        <div class="form-control border-0 bg-light rounded-pill px-4 py-2 text-muted text-sm font-roboto" style="cursor:pointer;">
          Share your botanical wisdom, {{ currentUser?.displayName }}...
        </div>
      </div>
      <button class="btn btn-primary d-flex align-items-center justify-content-center p-0 rounded-circle shadow-sm" style="width:48px; height:48px;" aria-label="Create post">
    ...
    <div v-else class="p-4 p-md-5 position-relative animate-fade-up" style="overflow:visible;">
      ...
      <div class="row g-4">
        ...
        <div class="col-12 col-lg-8" style="overflow:visible;">
          ...
          <div class="mb-3">
            <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Content</label>
            <RichTextEditor
              v-model="content"
          ...
          >
            ...
            <div class="row g-3" style="overflow:visible;">
              <div class="col-6 col-md-4">
                <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Rose Color</label>
                <input v-model="color" type="text" class="form-control rounded-pill border-2" placeholder="e.g. Peach Pastel" />
              </div>
              <div class="col-6 col-md-4">
                <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Fragrance Profile</label>
                <input v-model="fragrance" type="text" class="form-control rounded-pill border-2" placeholder="e.g. Citrus Amber" />
              </div>
              <div class="col-6 col-md-4">
                <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Blooming Season</label>
                <input v-model="bloomingSeason" type="text" class="form-control rounded-pill border-2" placeholder="e.g. Summer" />
              </div>
              ...
              <div class="col-6 col-md-4" style="position:relative; z-index:130;">
                <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Strength</label>
                <div class="custom-select-wrapper" v-click-outside="() => activeDropdown = null">
                  <div class="select-display-custom rounded-pill border-2" @click.stop="toggleDropdown('create-strength')">
                    {{ strengthOptions.find((o) => o.value === strength)?.label || 'Select...' }}
                    <span class="material-symbols-outlined transition-base" :class="{ 'rotate-180': activeDropdown === 'create-strength' }">expand_more</span>
                  </div>
                  <transition name="fade">
                    <div class="dropdown-menu-custom" v-if="activeDropdown === 'create-strength'" style="z-index:9999; position:absolute;">
                      <div
                        v-for="opt in strengthOptions" :key="opt.value"
              ...
              <div class="col-6 col-md-4" style="position:relative; z-index:120;">
                <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Thorn Level</label>
                <div class="custom-select-wrapper" v-click-outside="() => activeDropdown = null">
                  <div class="select-display-custom rounded-pill border-2" @click.stop="toggleDropdown('create-thorn')">
                    {{ thornOptions.find((o) => o.value === thornLevel)?.label || 'Select...' }}
                    <span class="material-symbols-outlined transition-base" :class="{ 'rotate-180': activeDropdown === 'create-thorn' }">expand_more</span>
                  </div>
                  <transition name="fade">
                    <div class="dropdown-menu-custom" v-if="activeDropdown === 'create-thorn'" style="z-index:9999; position:absolute;">
                      <div
                        v-for="opt in thornOptions" :key="opt.value"
                        class="dropdown-item-custom"
              ...
              <div class="col-6 col-md-4" style="position:relative; z-index:110;">
                <label class="form-label font-roboto fw-bold text-sm text-uppercase small">Ideal Environment</label>
                <div class="custom-select-wrapper" v-click-outside="() => activeDropdown = null">
                  <div class="select-display-custom rounded-pill border-2" @click.stop="toggleDropdown('create-ideal')">
                    {{ idealOptions.find((o) => o.value === idealFor)?.label || 'Select...' }}
                    <span class="material-symbols-outlined transition-base" :class="{ 'rotate-180': activeDropdown === 'create-ideal' }">expand_more</span>
                  </div>
                  <transition name="fade">
                    <div class="dropdown-menu-custom" v-if="activeDropdown === 'create-ideal'" style="z-index:9999; position:absolute;">
                      <div
                        v-for="opt in idealOptions" :key="opt.value"
                        class="dropdown-item-custom"
        ...
        <div class="col-12 col-lg-4">
          ...
          <div class="mb-4">
            <label class="form-label font-roboto fw-bold text-sm text-uppercase small mb-2 d-block">Post Visibility</label>
            <div class="create-bar__visibility-toggle d-flex rounded-4 border overflow-hidden shadow-sm">
              <button
                type="button"
          ...
          <div class="mb-4">
            ...
            <div class="btn-group w-100 mb-3 shadow-sm rounded-pill overflow-hidden border">
              <button type="button" class="btn btn-sm py-2 fw-bold" :class="imageMode === 'url' ? 'btn-primary' : 'btn-white'" @click="imageMode = 'url'">URL</button>
              <button type="button" class="btn btn-sm py-2 fw-bold" :class="imageMode === 'upload' ? 'btn-primary' : 'btn-white'" @click="imageMode = 'upload'">Upload</button>
            </div>

            <div v-if="imageUrl" class="rounded-4 overflow-hidden mb-3 border shadow-sm ratio ratio-1x1 position-relative">
              <img :src="imageUrl" class="object-fit-cover w-100 h-100" alt="Preview" />
              <button class="btn btn-dark btn-sm rounded-circle position-absolute top-0 end-0 m-2 p-1" @click="imageUrl = ''" aria-label="Remove image">
                <span class="material-symbols-outlined fs-6">close</span>
              </button>
            </div>

            <div v-else>
              <input v-if="imageMode === 'url'" v-model="imageUrl" type="url" class="form-control rounded-pill border-2 p-3 px-4 shadow-sm" placeholder="https://example.com/rose.jpg" />
              <label v-else class="btn btn-outline-secondary rounded-pill w-100 py-3 fw-bold border-2 d-flex align-items-center justify-content-center gap-2 cursor-pointer" style="border-style:dashed;">
          ...
          <!-- Error -->
          <div v-if="error" class="text-danger small mb-3 text-center fst-italic animate-fade-up" role="alert">{{ error }}</div>
