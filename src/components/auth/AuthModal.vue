...
export default {
  ...
  data() {
    return {
      ...
      password: '',
      displayName: '',
      // Toggle for the password input type (password/text).
      showPassword: false,
      // Tracks form submission state to prevent double-clicks.
      isSubmitting: false,
      // Validation and API error feedback.
      error: '',
    }
  },
  ...
  methods: {
    ...
    /**
     * Handles the authentication request asynchronously.
...
<template>
  <teleport to="body">
    <transition name="fade">
      ...
      >
        <div
          class="auth-modal frosted-glass rounded-5 shadow-2xl p-4 p-md-5 mw-480 position-relative animate-fade-up border border-white mx-3"
        >
          <!-- Close button -->
          <button
            class="position-absolute top-0 end-0 m-4 btn btn-link text-dark opacity-50 p-2"
            @click="close"
          ...
          <header class="text-center mb-4">
            <div
              class="footer-logo bg-primary text-white rounded-circle p-2 d-inline-flex align-items-center justify-content-center mb-2 shadow-sm"
            >
          ...
          <form @submit="checkForm" novalidate>
            <div class="mb-3">
              ...
              <input
                ...
                id="username"
                type="text"
                class="form-control rounded-pill border-2 p-3 px-4 shadow-sm font-roboto"
                placeholder="flower_lover_99"
                :disabled="isSubmitting"
              />
            </div>
            ...
            <div class="mb-3 position-relative">
              ...
              <button
                type="button"
                class="btn btn-link position-absolute end-0 top-50 translate-middle-y me-3 mt-3 text-muted"
            ...
            <div
              v-if="error"
              class="alert alert-danger rounded-4 py-2 text-sm text-center mb-4 animate-fade-up"
            >
              {{ error }}
            </div>
            ...
            <button
              type="submit"
              class="btn btn-primary w-100 rounded-pill py-3 fw-bold shadow-lg mb-3 text-uppercase ls-1"
              :disabled="isSubmitting"
            >
              <span
                v-if="isSubmitting"
              ...
              >
                {{
                  mode === 'login'
                    ...
                    : 'Already part of the garden? Login'
                }}
              </button>
            </div>
          </form>
